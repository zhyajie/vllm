...
class AiterMLAImpl(MLACommonImpl[AiterMLAMetadata]):
    def __init__(
        ...
        **mla_args,
        ...
        # Flag to ensure we only save debug tensors once
        self._debug_tensors_saved = False
    ...
    def _forward_decode(
        ...
        if self.num_heads == 8:
            ...
            if (not self._debug_tensors_saved and 
                ...
                if parallel_state.get_world_group().is_first_rank:
                    ...
                    final_path = os.path.join(save_dir, 'mla_decode_inputs.pt')
                    
                    print(f"\n{'='*80}", flush=True)
                    print(f"[Rank 0] Detected kv_indptr[1]=8 !!!", flush=True)
                    print(f"[Rank 0] Saving tensors to {final_path}", flush=True)
                    ...
                    try:
                        ...
                        # Clone tensors to CPU immediately
                        data_to_save = {
                            'q': q_reshaped.detach().cpu().clone(),
                            'k_buffer': kv_buffer.detach().cpu().clone(),
                            'o': o.detach().cpu().clone(),
                            'kv_indptr': kv_indptr.detach().cpu().clone(),
                            'block_tables': attn_metadata.decode.block_table.detach().cpu().clone(),
                            'kv_lora_rank': self.kv_lora_rank,
                            'attn_logits': attn_logits.detach().cpu().clone(),
                            'attn_lse': attn_lse.detach().cpu().clone(),
                        ...
                        with tempfile.NamedTemporaryFile(mode='wb', delete=False, 
                                                        dir=save_dir, suffix='.tmp') as tmp_file:
                        ...
                        if tmp_size < 1000:  # Sanity check: file should be at least 1KB
                            raise RuntimeError(f"Temp file too small: {tmp_size} bytes")
                        ...
                        # Final verification by attempting to load
                        try:
                            test_load = torch.load(final_path, map_location='cpu')
                            assert 'q' in test_load and 'k_buffer' in test_load
                            del test_load
                        except Exception as e:
                            raise RuntimeError(f"File verification failed: {e}")
                        
                        file_size = os.path.getsize(final_path)
                        print(f"[Rank 0] ✓✓✓ SUCCESS: Saved to {final_path}", flush=True)
                        print(f"[Rank 0] ✓✓✓ File size: {file_size / 1024 / 1024:.2f} MB", flush=True)
                        print(f"[Rank 0] ✓✓✓ File integrity verified", flush=True)
                        save_success = True
                        
                    except Exception as e:
                        print(f"[Rank 0] ✗✗✗ FAILED to save tensors: {e}", flush=True)
                        import traceback
                        traceback.print_exc()
                    ...
                    # Additional safety delay after save completion
                    if save_success:
                        print(f"[Rank 0] Waiting 2 seconds to ensure file is fully synced...", flush=True)
                        time.sleep(2)
                        print(f"[Rank 0] Save complete, proceeding...", flush=True)
                        print(f"{'='*80}\n", flush=True)
                else:
                    # Other ranks: wait for rank 0 to complete saving
                    print(f"[Rank {rank}] Detected kv_indptr[1]=8, waiting for Rank 0 to save...", flush=True)
                    time.sleep(3)  # Wait 3 seconds to give rank 0 enough time
                    print(f"[Rank {rank}] Wait complete, proceeding...", flush=True)
                
                # Mark as saved (across all ranks)
                self._debug_tensors_saved = True
                
                # Optional: Add a barrier if using distributed training
                # This ensures all ranks are synchronized before proceeding
                if world_size > 1:
                    try:
                        torch.distributed.barrier()
                        if rank == 0:
                            print(f"[Rank 0] All ranks synchronized after save", flush=True)
                    except Exception as e:
                        print(f"[Rank {rank}] Warning: barrier failed: {e}", flush=True)
